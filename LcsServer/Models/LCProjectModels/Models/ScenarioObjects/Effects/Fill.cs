...
namespace LcsServer.Models.LCProjectModels.Models.ScenarioObjects.Effects;
 ...
 /// <summary>
    ...
    /// </summary>
    public class Fill : BaseCyclicEffect
    {
        ...
        private float[] _fillAngles;
        private bool _isRandomColor;
        private int _blurFactor;
        private bool _isRevertEven;
        ...
        public event EventHandler IsRevertEvenChanged;
        ...
        public Fill(int id, string name, int parentId, FillStyles fillStyle, int angle, CompositeColor backgroundColor,
            CompositeColor compColor, InOutDirection direction, bool isRandomAngle, Rotation rotation, long totalTicks,
            bool isRandomColor, int repeat, long riseTime, long fadeTime, long cycleRiseTime, long cycleFadeTime,
            float dimmingLevel, int blurFactor, bool isRevertEven) : this()
        {
            Id = id;
            Name = name;
            _parentId = parentId;
        ...
        /// Тип
        /// </summary>
        public override Enum Type => FillingTypes.Fill;
        ...
        public bool IsRevertEven
        {
            get => _isRevertEven;
            set
            {
                if (_isRevertEven == value)
                {
                    return;
                }

                _isRevertEven = value;
                IsRevertEvenChanged?.Invoke(this, EventArgs.Empty);
            }
        }
        ...
        protected override Effect Clone(IPlayingEntity parent)
        {
            var clone = new Fill
            {
                ...
                Rotation = _rotation,
                IsRandomAngle = _isRandomAngle,
                Angle = _angle,
                CompColor = _compColor,
                IsRandomColor = _isRandomColor,
                Repeat = Repeat,
        ...
        {
            float tmp = ticks * Repeat / (float)TotalTicks - LCMath.Eps;
            int period = (int)tmp;
            float percent = tmp - period;
            
            percent = percent.Clamped(0, 1.0f);

            float totalRiseOrFadeLevel = GetCycleRiseOrFadeLevel(ticks) * riseOrFadeLevel;

            switch (_fillStyle)
            {
                case FillStyles.OneSide:
                    OneSide(frame, percent, period, totalRiseOrFadeLevel);
                    break;
                case FillStyles.TwoSides:
                    TwoSides(frame, percent, period, totalRiseOrFadeLevel);
                    break;
                case FillStyles.Rect:
                    Rect(frame, percent, period, totalRiseOrFadeLevel);
                    break;
                case FillStyles.Rhombus:
                    Rhombus(frame, percent, period, totalRiseOrFadeLevel);
                    break;
                case FillStyles.Circle:
                    Circle(frame, percent, period, totalRiseOrFadeLevel);
                    break;
                case FillStyles.Fan:
                    Fan(frame, percent, period, totalRiseOrFadeLevel);
                    break;
                default:
        ...
        private void OneSide(EffectFrame frame, float percent, int period, float level)
        {
            var x2 = frame.Width;
            var y2 = frame.Height;

            bool needRevert = period % 2 == 1 && _isRevertEven;
            if (needRevert)
            {
                percent = 1 - percent;
            }
            float locAngle = _compositeAngle;
            if (Repeat > 1 && IsRandomAngle)
            {
                locAngle = needRevert ? _fillAngles[period - 1]  : _fillAngles[period];
            }
        ...
        private void TwoSides(EffectFrame frame, float percent, int period, float level)
        {
            bool needRevert = period % 2 == 1 && _isRevertEven;
            if (needRevert)
            {
        ...
        private void Rect(EffectFrame frame, float percent, int period, float level)
        {
            bool needRevert = period % 2 == 1 && _isRevertEven;
            if (needRevert)
            {
        ...
        private void Rhombus(EffectFrame frame, float percent, int period, float level)
        {
            bool needRevert = period % 2 == 1 && _isRevertEven;
            if (needRevert)
            {
        ...
        private void Circle(EffectFrame frame, float percent, int period ,float level)
        {
            bool needRevert = period % 2 == 1 && _isRevertEven;
            if (needRevert)
            {
        ...
        private void Fan(EffectFrame frame, float percent, int period, float level)
        {
            bool needRevert = period % 2 == 1 && _isRevertEven;
            if (needRevert)
            {
            ...
            if (Repeat > 1 && IsRandomAngle)
            {
                cAngle = needRevert ? _fillAngles[period - 1] : _fillAngles[period];
            }
        ...
        {
            _currCompositeColor = IsRandomColor ?
                new CompositeColor(LCRandom.RandomNextByte(), LCRandom.RandomNextByte(), LCRandom.RandomNextByte()) :
                _compColor;
        }
        ...
        private void UpdateCompositeAngle()
        {
            if (IsRandomAngle)
            {
                ...
                _fillAngles = new float[Repeat];
                for (int i = 0; i < Repeat; i++)
                {

                    if (i == 0)
                    {
                        _fillAngles[i] = _angle;
                    }

                    _fillAngles[i] = rnd.Next(359);
                }
            }
            else
            {
